...
CPS-0031 asks that urgent transactions have a way to avoid value-destroying delay when they compete with traffic that has no time sensitivity. Retained value measures exactly the value that delay destroys, so this goal is judged on the retained-value results. The mechanism preserves more urgent-class value than the flat fee at every contended load: +7.16 percentage points under severe congestion in the thousand-seed replication, +3.69 at mid load, +8.57 under EB-capacity stress, and +8.15 points of overall retained value under the launch-day profile. Urgent-class mean latency falls with it, from 2.97 to 2.51 blocks under severe congestion. Two channels carry the improvement. The reserved Ranking Block path gives urgent-paying transactions an earlier inclusion opportunity, and the rising standard quote makes low-surplus demand decline to submit during congestion, which leaves capacity for the demand that values inclusion most. The cost sits at low load, where overall retained value is 0.40 percentage points below flat fee while any urgent-class effect is bounded within roughly ±0.15 points. CPS-0031 lists guaranteed value retention as a non-goal, and this mechanism does not promise it: the improvement is a measured shift in the distribution of outcomes, detailed in [Experimental evidence](#experimental-evidence).
