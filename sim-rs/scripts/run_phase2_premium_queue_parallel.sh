...
usage() {
  cat <<'USAGE'
Run the phase-2 premium-queue paper-like sweep in parallel, one seed per shard.

Usage:
  scripts/run_phase2_premium_queue_parallel.sh run [--label <name>] [--output-root <dir>]
  scripts/run_phase2_premium_queue_parallel.sh resume <batch-dir>

Commands:
  run                    Build the release binary, launch all premium-queue shards
...
Options:
  --label <name>         Optional batch label for `run`.
                         Default: phase2-premium-queue-paper-like
...
DEFAULT_LABEL="phase2-premium-queue-paper-like"
...
shard_name_from_config() {
  ...
  name="${name%.yaml}"
  name="${name#phase-2-premium-queue-paper-like-}"
  printf "%s\n" "${name}"
}
...
run_batch() {
  ...
  local configs=("${SHARD_CONFIG_DIR}"/phase-2-premium-queue-paper-like-*.yaml)
