...
The simulator at `sim-rs/` is the primary phase-2 evidence-generation tool. It implements an approximation of the spec for tractability. Known divergences between simulator and spec:
...
| Logical / tag-based partition (placeholder) | Two-lane lane preference stored as `Transaction::tier_preference` (priority or standard); selection scans by lane preference, no partition representation distinct from per-tx tag |
| Capacity-weighted aggregate utilisation over a window for single-lane EIP-1559 (placeholder) | Per-priced-block fill rate against that block's own capacity (`block_bytes / block_capacity`), no windowing or capacity weighting. RB updates and EB updates fire independent EIP-1559 steps with very different denominators (90KB vs 12MB). Pre-2026-05-06 evidence reflects this older signal; the controller's response confounds demand and block-type cadence. Polina's prototype draft uses per-RB updates against a fixed `targetBytesPerRb` instead, which solves the cadence problem but introduces a bimodal settlement-step distribution — neither sim nor prototype yet implements the spec's capacity-weighted approach. The two-lane priority controller (RB-reserved) is unaffected: its priority capacity is uniform per block, so the simulator's per-block `min(priority_bytes, rb_capacity) / rb_capacity` formulation already matches the spec. |

Re-validation of the discarded fully-static variant requires the simulator's invalidation/refund model to align with the spec. The other divergences are acceptable approximations for welfare-metric purposes but should be reconciled before the simulator's evidence is used to argue for specific implementation choices in the on-ledger prototype.
