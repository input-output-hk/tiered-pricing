...
The prototype follows the recommended construction at the two transport boundaries that were previously experimental. It FIFO-merges the entire urgent remainder into the Endorser Block, without an express reserve for the next Ranking Block. A certificate-carrying Ranking Block is payload-free: its unresolved transaction sequence is empty, and resolution applies only the certified Endorser Block cargo. This keeps the prototype aligned with the Linear Leios block-body construction and gives every transaction in the resolved block the same certified-delivery semantics.
