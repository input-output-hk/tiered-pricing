...
usage() {
  cat <<'USAGE'
Run the phase-2 Stage A sweep by launching one suite shard per top-level axis job.

Usage:
  scripts/run_phase2_stage_a_parallel.sh run [--label <name>] [--output-root <dir>]
  scripts/run_phase2_stage_a_parallel.sh resume <batch-dir>

Commands:
  run                    Build the release binary, launch all Stage A shard suites
...
Options:
  --label <name>         Optional batch label for `run`.
                         Default: phase2-stage-a-parallel
  --output-root <dir>    Parent directory for new batch directories.
                         Default: output/experiment-batches
  -h, --help             Show this help.

Notes:
  - Each shard gets its own suite output root under <batch-dir>/shards/<shard-name>/.
...
DEFAULT_LABEL="phase2-stage-a-parallel"
...
run_batch() {
  ...
  local configs=("${SHARD_CONFIG_DIR}"/phase-2-stage-a-*.yaml)
