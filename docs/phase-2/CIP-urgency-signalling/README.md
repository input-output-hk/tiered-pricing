...
The urgency rule is enforced by the ledger: every transaction in a valid Ranking Block must carry on-ledger fee authorisation covering the applicable urgent quote. In simulation the mechanism preserves more urgent-class transaction value under severe congestion than today's flat fee, where retained value means the modelled delay-discounted gross transaction value remaining at inclusion, before fees. Across the other four headline workloads, the most delay-sensitive transactions showed no statistically detectable difference from that baseline at low load, improved at mid and EB-stressing load, and under a launch-day surge the mechanism retains more value overall. At light load the whole mechanism very slightly reduces overall retained value, because transactions that remain on the standard path wait longer while Endorser Blocks fill. Exact figures are given in the Rationale.
...
The parameter values, the grid points tested, and the loads at which each was stressed are tabulated in the announcement-threshold section.
