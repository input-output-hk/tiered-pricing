...
usage() {
  cat <<'USAGE'
Run the phase-2 EIP-1559 priority-lane paper-like comparison in parallel, one seed per shard.

Usage:
  scripts/run_phase2_eip1559_priority_lane_parallel.sh run [--label <name>] [--output-root <dir>]
  scripts/run_phase2_eip1559_priority_lane_parallel.sh resume <batch-dir>

Commands:
  run                    Build the release binary, launch all priority-lane shards
...
Options:
  --label <name>         Optional batch label for `run`.
                         Default: phase2-eip1559-priority-lane-paper-like
...
DEFAULT_LABEL="phase2-eip1559-priority-lane-paper-like"
...
shard_name_from_config() {
  ...
  name="${name%.yaml}"
  name="${name#phase-2-eip1559-priority-lane-paper-like-}"
  printf "%s\n" "${name}"
}
...
run_batch() {
  ...
  local configs=("${SHARD_CONFIG_DIR}"/phase-2-eip1559-priority-lane-paper-like-*.yaml)
