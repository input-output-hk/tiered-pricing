...
manifest="config/sweeps/cert-qualification-smoke.json"
out_dir="sweep-results/cert-qualification-smoke"
seed_start=450
seeds=10
slots=2000
...
usage() {
  cat <<'EOF'
usage: ./scripts/run_cert_qualification_smoke.sh [--out DIR] [--seed-start N] [--seeds N] [--slots N]

Run a paired two-arm ablation of where the EB threshold-or-age qualification
binds, on the adopted configuration:

  adopted          historical announcement-time qualification: the earliest
                   below-threshold certificate lands one RB later than the
                   specified rule allows
  qualify-at-cert  the specified certificate-inclusion rule: a prudent
                   producer announces one RB earlier, and certification
                   enforces the qualification with the certifying RB counted
...
python3 scripts/compare_cert_qualification_smoke.py \
  --root "$out_dir" \
  --manifest "$manifest" \
...
echo "cert-qualification ablation complete in ${SECONDS}s:"
