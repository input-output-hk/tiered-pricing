...
- [x] **VIZ-06** — The site runs locally on the developer's machine. Whether that means a static bundle, a build script, or a local dev server is a discuss-phase decision; the requirement is that one documented command produces a browsable site. Acceptance: a single command (in `CLAUDE.md` or workstream README) gets a fresh dev environment to a viewable viz site against the current `sim-rs/output/` tree. **Closed by Plan 01-06: `python sim-rs/scripts/viz/build.py --serve` is documented in `sim-rs/scripts/viz/README.md` (201 lines — command, all six argparse flags, three-tier output layout, Notes covering every Pitfall 1-8 / CRITICAL LANDMINE, annual refresh recipe covering both vendored Observable Plot AND its D3 peer-dependency bundle) and breadcrumbed from `CLAUDE.md` `### Visualising suite results` under `## Running the suites`. End-to-end visual verification confirmed by the orchestrator against the live `sim-rs/output/` tree (12 phase-2 suites loaded, three Plot chart panes rendered, cross-seed overlay green, console clean). 18/18 viz tests green.**
