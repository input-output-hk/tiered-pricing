...
<!-- PORTABILITY: relative link into abstract-sim-hs breaks in the CIPs repo; copy thr-k10.json into this CIP folder as a supporting file, or use a commit-pinned permalink -->
...
EB construction operates the same way Praos block construction operates today: we consult the canonical queue in a FIFO manner.
...
<!-- PORTABILITY: the per-seed scalars link below is relative; replace with a commit-pinned permalink before the CIPs-repo PR -->
...
<!-- PORTABILITY: the D16 evidence record link below is relative; replace with a commit-pinned permalink before the CIPs-repo PR -->
...
<!-- PORTABILITY: the canonical integration evidence link below is relative; replace with a commit-pinned permalink before the CIPs-repo PR -->
...
Since we want to enforce the rule that only transactions paying a sufficient fee to enter the urgent lane may be admitted to Praos blocks, we must make [ledger changes](https://github.com/IntersectMBO/formal-ledger-specifications/compare/master...polina/dynamic).
...
<!-- PORTABILITY: relative figure links (this figure and the next); copy the figures into this CIP folder and relink before the CIPs-repo PR -->
