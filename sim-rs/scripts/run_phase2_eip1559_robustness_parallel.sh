...
usage() {
  cat <<'USAGE'
Run the phase-2 EIP-like robustness sweep in parallel, one seed per shard.

Usage:
  scripts/run_phase2_eip1559_robustness_parallel.sh run [--label <name>] [--output-root <dir>]
  scripts/run_phase2_eip1559_robustness_parallel.sh resume <batch-dir>

Commands:
  run                    Build the release binary, launch all robustness shards
...
Options:
  --label <name>         Optional batch label for `run`.
                         Default: phase2-eip1559-robustness-paper-like
  --output-root <dir>    Parent directory for new batch directories.
                         Default: output/experiment-batches
  -h, --help             Show this help.

Notes:
  - Each seed shard runs both paper-like moderate and paper-like congested demand.
  - The baseline case inside each job is denom8/target50.
...
DEFAULT_LABEL="phase2-eip1559-robustness-paper-like"
...
shard_name_from_config() {
  ...
  name="${name%.yaml}"
  name="${name#phase-2-eip1559-robustness-paper-like-}"
  printf "%s\n" "${name}"
}
...
run_batch() {
  ...
  local configs=("${SHARD_CONFIG_DIR}"/phase-2-eip1559-robustness-paper-like-*.yaml)
