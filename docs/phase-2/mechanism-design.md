...
| Chain-derived controller implementation pattern (closes WR-1 orphan-block contamination) | **Resolved at WR-1 fix (2026-05-14)** — chain-derived pattern adopted per spike 007. Every `LinearRankingBlock` carries `derived_quote` as a pure function of the parent block's chain-derived state, matching EIP-1559's stateless pattern. See spike 007 and `.planning/chain-derived-controller-PLAN.md`. |
| Controller cadence = one step per canonical block per EIP-1559 (no extra step on deferred EB validation) | **Resolved 2026-05-14** — the pre-2026-05-14 accumulator implementation effectively performed two controller steps per RB-EB pair via separate `apply_priced_block` and `apply_eb_priced_block` calls. This was an unintentional divergence from the spec's per-block-cadence intent, characterized post-hoc as an "implementation variant" rather than spec behavior. The chain-derived refactor advances the controller exactly once per canonical block, matching textbook EIP-1559 and this spec. Family B (1-step-per-canonical-block) is the committed publication mechanism; see `.planning/chain-derived-bug2-investigation.md` and `.planning/family-b-decision-2026-05-14.md`. |
...
**Cadence is one controller step per canonical block, per EIP-1559.** The chain-derived `derived_quote` advances exactly once per canonical Ranking Block — at production, as a pure function of the parent block's chain-derived state plus the samples emitted by the parent. **Deferred Endorser Block (EB) validation does NOT trigger an additional controller step.** When an EB whose validation was deferred completes validation at a node, the node updates its sample cache for the certifying RB so that descendants can fold the EB sample into their own (single) step at production time — but no controller step fires at the moment of deferred-EB validation itself. This is the textbook EIP-1559 semantics: one update per canonical block, never more.

(Historical note: an earlier accumulator-based implementation effectively performed two controller steps per RB-EB pair via separate `apply_priced_block` and `apply_eb_priced_block` calls. That was an unintentional implementation divergence from this spec's per-block-cadence intent and has been corrected by the chain-derived refactor. See `.planning/chain-derived-bug2-investigation.md` for the diagnosis and `.planning/family-b-decision-2026-05-14.md` for the publication commitment.)
...
| Derived-quote cadence | Per priced block; each `LinearRankingBlock` carries its own `derived_quote` computed at production as a pure function of the parent's chain-derived state | [§"Open calibration choices"](#open-questions) (cadence) | Per-RB or per-epoch alternatives require a rewrite of the simulator's chain-derived production path in `linear_leios.rs`; intrusive. |
...
| Default actor `max_fee_policy` | `ScaledOverLaneQuote { numerator: 4, denominator: 1 }` — i.e. 4× quote-drift headroom on `max_fee_lovelace` | [§"Open questions"](#open-questions) (Intent vs fee, partial) | Per-component override in a demand YAML. `paper_like_mispriced.yaml` does this for the short-half-life component (`{1, 1}`, zero headroom). |
...
| `rb-generation-probability` and `default-slots` | 0.05 (Cardano-realistic) and 2000 respectively | linear-Leios endorsement window: spec is silent, but `try_generate_rb`'s `earliest_endorse_time` check requires `slot >= parent_rb.slot + header_diffusion x 3 + linear_vote_stage_length + linear_diffuse_stage_length` (about 13 slots). Probability 0.05 gives ~20-slot expected RB cadence, clearing the window; 2000 slots gives parity with `pricing-sim-base` run length. | An earlier revision pinned `rb-prob = 1.0` for uniform tx-bearing-block-per-slot output; that combination prevented EB endorsement entirely. See [calibration-fix-postmortem.md](calibration-fix-postmortem.md) for the full explanation. *Re-calibrating*: any rb-prob >= ~0.077 reintroduces the bug (median RB gap drops below 13 slots); paired with `topology-single-producer.yaml`'s `stake: 100000` to preserve probability under VRF stake quantization. |
