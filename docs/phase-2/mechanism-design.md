...
| `multiplier_floor` in `phase-2-rb-scarcity` and `phase-2-urgency-inversion` | 4 (rather than the default 16 used by other suites' `*_x16` jobs) | [§"Calibration vs invariant"](#calibration-vs-invariant) | Raise the floor in those suites' pricing YAMLs; signals will weaken because fewer urgency components self-select into priority. The single-producer + `rb-generation-probability: 1.0` topology is the underlying constraint — at 16, priority demand stays too low to drift the controller in 200 slots. Full rationale in [m4-handoff.md §Decisions M4 made](m4-handoff.md). |
