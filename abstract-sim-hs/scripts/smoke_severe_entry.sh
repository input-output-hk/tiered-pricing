...
out_dir="sweep-results/severe-entry-smoke"
seeds=10
slots=2000

usage() {
  cat <<'EOF'
usage: ./scripts/smoke_severe_entry.sh [--out DIR] [--seeds N] [--slots N] [--manifest FILE]

Measure the urgent-class entry differential under severe-congestion load by
pairing flat fee with the canonical D16/K10 mechanism. By default this runs
seeds 0-9 for 2,000 slots (20 summary-only simulations), then writes
comparison.md and comparison.json with submitted counts, the derived
urgent-class submitted value, and the headline welfare metrics.
...
python3 scripts/compare_severe_entry.py \
