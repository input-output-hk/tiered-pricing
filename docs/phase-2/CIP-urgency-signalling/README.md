...
TODO: fold the standard-lane batching cost into the Incentives fairness discussion: the wait is now bounded at K Ranking Block intervals by the age escape, and at typical light load costs p95 roughly four slots over plain reservation. The thousand-seed replication in the Rationale prices the trade at low load: transactions staying standard wait a mean 57.96 slots against 34.25 under flat fee and overall retained value sits 0.40 ± 0.01 percentage points below flat fee, while urgent-lane transactions keep the flat-fee latency profile and the urgent demand class stays at statistical parity. Urgent transactions included via EB are refunded down to the standard quote; reference the fee change return mechanism CIP when drafted.
...
The recommended mechanism is both-dynamic-strict-threshold with a 5-sample signal window, calibrated at target utilisation 0.5 with max-change denominator 16. Under severe congestion it improves urgent retained value from 43.56% (flat fee) to 50.85% (+7.29 ± 1.29 percentage points, paired over ten seeds against a matched flat-fee control) and reduces urgent mean latency from 2.98 to 2.50 blocks. (All per-load figures in this paragraph are from the D16/K10 headline rerun below; the experiment report's comparison tables were generated on the denominator-8 anchor configuration. The parameter stress test found no statistically detectable welfare difference between the two calibrations at any swept load at baseline demand elasticity, with denominator 16 eliminating price shocks - the mid-load profile was not part of that sweep, so no D8/D16 comparison was measured there. Under an extreme high-value demand mix, denominator 8 retains ~2 percentage points more at a large stability cost; 16 remains the default on the asymmetry of those error modes, with 8 available inside the validated envelope should persistently steep demand emerge.) At mid load it beats flat fee by +3.69 ± 1.06 percentage points (ten of ten seeds), and under the EB-stressing load by +8.57 ± 2.24 (ten of ten). At low load - the regime where plain reservation regresses below flat fee - the EB threshold repairs the regression, leaving no statistically detectable difference from the flat-fee baseline (+0.47 ± 1.62 over ten seeds; the thousand-seed replication below tightens the bound to -0.03 ± 0.12). Under the launch-day profile it beats flat fee by +8.15 ± 2.04 percentage points of overall offered value (ten of ten seeds), through admission rather than latency: the rising standard quote makes low-surplus demand decline to submit, and what remains is included with near-certainty rather than decaying in a jammed mempool.
...
Under severe congestion the headline is confirmed and sharpened. Urgent retained value improves from 43.56% to 50.72%, a paired difference of +7.16 percentage points (95% CI [+7.06, +7.26]) with all 1,000 seeds improving; urgent mean latency falls from 2.97 to 2.51 blocks, again in every seed; and overall retained value rises by +0.38 percentage points (95% CI [+0.37, +0.39]).

At low load the parity claim strengthens by an order of magnitude. The urgent retained-value difference is -0.03 percentage points with a 95% CI of [-0.15, +0.10], so any urgent-lane effect at this load is bounded within roughly ±0.15 percentage points; urgent service rate is likewise indistinguishable, and urgent mean latency is marginally lower (-0.29 slots, 95% CI [-0.47, -0.11]). Urgent-class users submit about 7.6% more transactions than under flat fee, at unchanged retention and service rate: demand that would have declined to enter now does, because a reserved lane exists.

The larger sample also resolves a cost invisible at ten seeds: overall retained value at low load sits 0.40 percentage points below flat fee (95% CI [-0.41, -0.39], flat fee better in 982 of 1,000 seeds; ratios count value whose fate resolved within the horizon). The split between the lanes is clean. The urgent lane loses nothing: its retention, service rate, and latency all sit at the parity bounds reported above. The standard lane carries the entire cost: standard transactions wait a mean of 57.96 slots against 34.25 under flat fee (2.99 against 1.78 blocks) while pooling for Endorser Blocks worth their certificate, and the value that decays over that extra wait is the 0.40. This quantifies the wait the Specification accepts: the announcement threshold's low-load price is 0.40 ± 0.01 percentage points of overall retained value, in exchange for the +7.16-point urgent improvement under severe congestion. Certificate overhead moves in the mechanism's favour at this load: mean certified Endorser Block payload rises from 95.5 KiB under flat fee to 116.5 KiB, so certificates are rarer per byte of standard traffic delivered, and each delivers more payload than the Ranking Block it consumes.
...
* Delay enforcement is unspecified in the paper. Specifying it required new ledger machinery to track in-limbo transactions through block headers and ledger state, raised unresolved DoS questions around when they are validated, and mapped awkwardly onto linear-Leios (should a transaction destined for the higher-latency Endorser Block be delayed equally?)
