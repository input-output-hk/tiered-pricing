...
- A withheld certificate stalls endorser-block delivery until votes resume, isolating the certification dependency described in the Specification.

The prototype runs the recommended construction in full: the controller calibration (target utilisation 0.5, max-change denominator 16), the 5-sample and 20-block signal windows (bytes and execution units, larger ratio), no cross-lane floor, the urgent lane's 2× initial coefficient, admission one worst-case controller step ahead, the announcement byte threshold, the K = 10 announcement age escape, settlement by delivery (an urgent transaction delivered through a certified endorser block is charged the standard quote, the excess refunded, and every fee-cap check uses the max of the two quotes), and the endorser block as the first-come merge of the standard queue and the urgent overflow (its riders). The prototype adds one deliberate selection variant, flagged for review: the node holds one ranking block's worth of urgent backlog out of the merge, so the express lane never forges empty while its service is being bought — the held transactions keep the instant delivery they are bidding the premium for. The merge is kept safe node-side: a node drops an announced endorser block's transactions from its mempool as soon as it stores its contents, so no later ranking block can carry one of them a second time.

The one simplification left is inherited from the linear-Leios prototype base, not from this mechanism: the certificate is a stub — committee votes are cast and counted, but no aggregate proof is built or verified. The pricing consumes only the fact "certified at this block", so a real certificate slots in without changing any rule above.
