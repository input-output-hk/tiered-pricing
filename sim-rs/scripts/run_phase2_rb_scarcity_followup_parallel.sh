...
usage() {
  cat <<'USAGE'
Run the phase-2 RB-scarcity paper-like follow-up sweep in parallel, one seed per shard.

Usage:
  scripts/run_phase2_rb_scarcity_followup_parallel.sh run [--label <name>] [--output-root <dir>]
  scripts/run_phase2_rb_scarcity_followup_parallel.sh resume <batch-dir>

Commands:
  run                    Build the release binary, launch all RB-scarcity follow-up shards
...
Options:
  --label <name>         Optional batch label for `run`.
                         Default: phase2-rb-scarcity-followup-paper-like
...
DEFAULT_LABEL="phase2-rb-scarcity-followup-paper-like"
...
shard_name_from_config() {
  ...
  name="${name%.yaml}"
  name="${name#phase-2-rb-scarcity-followup-paper-like-}"
  printf "%s\n" "${name}"
}
...
run_batch() {
  ...
  local configs=("${SHARD_CONFIG_DIR}"/phase-2-rb-scarcity-followup-paper-like-*.yaml)
