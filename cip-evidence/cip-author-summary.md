...
|---|---|---|---|
| Methodology | `audit-documents/methodology-overview.md` | Full document by repo Uniform Resource Locator (URL) — seven-element index + per-element prose + worked example tracing `menu_unreserved_priority_only_static_x4` seed=1 through the seven elements | Reference |
...
**Paste order:** The methodology overview is a one-page seven-element index with per-element prose and a worked example. The CIP cites it by repo Uniform Resource Locator (URL) rather than pasting content. The CIP's Methodology section should consist of: (a) a one-paragraph reference to the repo URL of `methodology-overview.md`; (b) a one-paragraph high-level summary of the seven elements (Purpose, State variables, Process overview, Design concepts, Initialisation, Input data, Submodels); (c) a one-sentence reference to the worked example tracing the canonical job-seed pair (`menu_unreserved_priority_only_static_x4` seed=1) through the seven elements.
...
**Paste order:** The audit carries seventeen `(value, source, date-retrieved)` triples in §"Topology and actor model" (calibration of the realistic-100 topology against epoch-582 mainnet), §"Pricing-controller calibration" (the four un-anchored controller knobs: window-length 32, multiplier-floor 4, multiplier-floor 16, lane-signal-source — each carries per-knob rationale), §"What lines up with mainnet" (the Ethereum Improvement Proposal 1559 (EIP-1559) core parameters that match Ethereum mainnet bit-exact: D=8, target=0.5, per-priced-block update cadence), and §"RB cadence" (ranking-block (RB) cadence calibration). The CIP cites specific calibration values by triple but does not paste the full audit; it links to the audit by repo Uniform Resource Locator (URL) and the §"Recommended disclosure statements" subsection.

**What "anchored" means here:** a calibration value is *anchored* when it cites a deployed-system datum. Of the seventeen triples in the audit, the ranking-block (RB) cadence + the EIP-1559 core parameters (D=8, target=0.5, per-priced-block update cadence) are anchored against deployed Cardano and Ethereum mainnet respectively; the window length 32, multiplier floor 4, multiplier floor 16, and lane-signal-source choices carry per-knob rationale paragraphs in `RSK-un-anchored-controller-knobs`. `audit-documents/cardano-realism-audit.md` §"Pricing-controller calibration" is the source-of-truth for which knobs are anchored versus disclosed.
...
**Source-of-truth:** `audit-documents/coverage-check.md` + `test-results/multi-seed-variance/results.md` + `test-results/multiplier-floor-16-companion/results.md` + `test-results/hash-diversity-gate/results.md`.
...
>
> **Backed by:**
> - **CLM-07** (`priority-only-un-reserved`): BCa 95% CI = `[+4.28e+09, +8.49e+09]`; median Δ retained_value = `+6.66e+09`; sign-coherence = `0.90`; distinct-hash = `20/20`; backing-job = `menu_unreserved_priority_only_static_x4` in `sim-rs/parameters/phase-2-sweep/suites/robustness-canonical-variance.yaml`.
> - **CLM-09** (`both-dynamic-un-partitioned`): BCa 95% CI = `[+5.65e+09, +1.09e+10]`; median Δ = `+7.95e+09`; sign-coherence = `0.90`; distinct-hash = `20/20`; backing-job = `menu_unreserved_both_dynamic_x4` in `sim-rs/parameters/phase-2-sweep/suites/robustness-canonical-variance.yaml`.
...
> **Headline Claim 2:** "Ranking-block-reserved (RB-reserved) two-lane mechanisms underperform single-lane EIP-1559 under the same calibration; this REFUTES the pre-robustness single-seed framing that 'two-lane mechanisms outperform single-lane EIP-1559', which holds only for the un-reserved variants."
>
> **Backed by:**
> - **CLM-06** (`priority-only-RB-reserved`): BCa 95% CI = `[-6.02e+09, -1.00e+09]`; median Δ = `-4.15e+09`; sign-coherence = `0.65`; distinct-hash = `20/20`; backing-job = `menu_rb_reserved_priority_only_static_x4` in `sim-rs/parameters/phase-2-sweep/suites/robustness-canonical-variance.yaml`. (Coverage-check verdict `WEAK` predates the robustness suites; the robustness N=20 CI excludes zero — the CIP author should cite the robustness-suite evidence at face value.)
> - **CLM-08** (`both-dynamic-partitioned`): BCa 95% CI = `[-5.95e+09, -8.87e+08]`; median Δ = `-4.15e+09`; sign-coherence = `0.65`; distinct-hash = `20/20`; backing-job = `menu_rb_reserved_both_dynamic_x4` in `sim-rs/parameters/phase-2-sweep/suites/robustness-canonical-variance.yaml`. (Same coverage-check-vs-robustness caveat as CLM-06.)
>
...
>
> **Backed by:**
> - robustness hash-diversity-gate report at `test-results/hash-diversity-gate/results.md`: TEST-03 (6 jobs × 20 seeds, all 20/20), TEST-04 (5 jobs × 20 seeds, all 20/20), TEST-07a (6 jobs × 5 seeds, all 5/5). Cross-cell Secure Hash Algorithm 256-bit (SHA-256) identity in TEST-07a is across-cell (does not violate within-cell gate per `RSK-hash-diversity-policy`'s disclosure-paragraph).
>
...
> This Cardano Improvement Proposal (CIP) uses four pricing-controller calibration values, none of which are anchored against deployed-system data. Per-knob rationale follows. **(a) Window length 32 priced blocks** was picked arbitrarily as a round number; the `phase-2-eip1559-smoothing` suite sweeps {16, 32, 64} to bracket the choice. **(b) Multiplier floor 4** in `phase-2-rb-scarcity` and `phase-2-urgency-inversion` is an internal calibration accommodation, chosen so urgency ≥ 2 components self-select into priority and surface controller drift in two specific suites; TEST-07a confirms regime-dependence at floor=16 (see Headline Claim 3 above). **(c) Multiplier floor 16** (the spec default) has no external anchor; welfare findings from the five suites with an `x16` variant are reported under the spec-stated assumption that 16 gives strong discrimination. **(d) Lane-signal-source choices** for un-reserved priority (option 1: `priority_paying_bytes / total_block_capacity`) and both-dynamic standard (`standard_paying_bytes / eb_referenced_txs_max_size_bytes` over endorser blocks; no standard sample on RB-reserved RBs) have no external anchor; the specification leaves both choices open at `mechanism-design.md` lines 207-211 and 238. Welfare findings from un-reserved priority + both-dynamic standard sides are conditional on these specific signal-source definitions; alternative signal sources were not exercised.
